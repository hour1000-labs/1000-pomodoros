...
export function isCountableFocusSession(session: FocusSession) {
  return (
    ...
    normalizeFocusedMinutes(session.focusedMinutes) >= MINIMUM_FOCUSED_MINUTES
  );
}

export function getCountableFocusSessions(sessions: readonly FocusSession[], journeyId?: string) {
...
export function getFocusedMinutes(sessions: readonly FocusSession[], journeyId?: string) {
...
export function getSessionsForLocalDate(sessions: readonly FocusSession[], date: Date) {
...
export function deriveProgressFromSessions(sessions: readonly FocusSession[], journeyId?: string) {
...
export function deriveJourneyProgress(
  ...
  return deriveTargetProgress(getFocusedMinutes(sessions, journey.id), journey.targetMinutes);
}
