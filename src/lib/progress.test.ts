...
describe('focus progress', () => {
  it('excludes sessions under five focused minutes', () => {
    ...
    expect(sessions.map(isCountableFocusSession)).toEqual([false, false, true]);
  ...
  it('selects sessions using the requested local calendar date', () => {
    ...
    expect(getSessionsForLocalDate(sessions, requestedDate)).toEqual([sessions[0]]);
  });
  ...
  it('derives target progress without maintaining a separate total', () => {
    ...
    expect(deriveJourneyProgress(journey, [createSession(25), createSession(50)])).toMatchObject({
      focusedMinutes: 75,
      targetMinutes: 100,
      targetPomodoros: 4,
      targetProgress: 0.75,
  ...
  it('derives the seeded 72% progress toward the 25-hour milestone', () => {
    expect(deriveTargetProgress(1_075, 25 * 60).targetPercentage).toBeCloseTo(71.67, 2);
  });
});
