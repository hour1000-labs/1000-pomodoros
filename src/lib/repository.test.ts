...
describe('localStorage repository', () => {
  ...
  it('persists onboarding, active session, timer, completion, and milestones', () => {
    ...
    expect(activeReload.status === 'ready' && activeReload.state.activeTimer).toEqual(
      activeTimer,
    )
    ...
    expect(
      repository.completeSession(completedSession, [earnedMilestone]).status,
    ).toBe('saved')
    ...
    expect(
      completedReload.state.focusSessions.find(
        ({ id }) => id === completedSession.id,
      ),
    ).toEqual(completedSession)
    expect(
      completedReload.state.milestones.find(
        ({ id }) => id === earnedMilestone.id,
      ),
    ).toEqual(earnedMilestone)
    expect(completedReload.state.lastCompletedSessionId).toBe(
      completedSession.id,
    )
  })
  ...
  it('finishes onboarding without creating duplicate Journeys or Next steps', () => {
    ...
    expect(
      result.state.journeys.filter(({ id }) => id === journey.id),
    ).toHaveLength(1)
    expect(
      result.state.nextSteps.filter(({ id }) => id === nextStep.id),
    ).toHaveLength(1)
  ...
  it('uses the preserved completed session Journey when replay input conflicts', () => {
    ...
    expect(result.state.lastActiveJourneyId).toBe(completedSession.journeyId)
    expect(
      result.state.journeys.find(({ id }) => id === completedSession.journeyId)
        ?.updatedAt,
    ).toBe(completedSession.endedAt)
    expect(
      result.state.journeys.find(({ id }) => id === otherJourney.id)?.updatedAt,
    ).toBe(otherJourney.updatedAt)
  })
