...
describe('LandingPage', () => {
  it('presents the exact promise and routes both primary actions to onboarding', async () => {
    ...
    expect(onboardingLinks.every((link) => link.getAttribute('href') === '/onboarding/journey')).toBe(true)
    expect(screen.getByRole('link', { name: 'See how it works' }).getAttribute('href')).toBe('#product-demonstration')
  })
  ...
  it('labels the seeded product demonstration and limits benefits to four', async () => {
    ...
    expect(within(demonstration).getByRole('heading', { level: 2, name: 'Learn guitar' })).toBeTruthy()
