...
function JourneyForm({ state }: { state: AppState }) {
  ...
  const exitAction = hasDraftData ? (
    <ConfirmDialog
      trigger={<Button variant="link" className="min-w-11 px-0 text-ink">Exit</Button>}
        ...
        <section className="w-full max-w-[42rem] lg:justify-self-end">
          ...
          <form noValidate onSubmit={handleSubmit}>
            ...
            <div className="min-h-7 pt-2">
              {showValidation ? (
                <p className="mb-0 text-sm font-bold text-pomodoro-red" id="journey-name-error" role="alert">
                  {validationError}
                </p>
              ) : null}
            </div>
