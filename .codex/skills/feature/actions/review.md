...
3. Evaluate each acceptance criterion separately:
...
4. Check for concrete bugs, regressions, accessibility problems, missing error
   handling, unrelated refactors, dependency changes, and scope creep.
5. Confirm every code change maps to an acceptance criterion or is strictly
   required to support one.
6. Do not mark verification items based on code inspection alone. Use evidence
   recorded by `test` or checks you actually run during this action.
7. Set status to `Ready to Commit` only when:
...
show a criterion-by-criterion result and finish with exactly one verdict:
`Ready to Commit` or `Needs Changes`.
